...
public interface AgencyRepository extends JpaRepository<AgencyEntity, Long> {
  ...
  @Query(
      value =
          "SELECT a.id AS id, a.name AS name, a.image AS image, "
              ...
              + "FROM agencies a WHERE a.is_active = true AND a.name LIKE %:search%",
      countQuery =
          "SELECT COUNT(*) FROM agencies a WHERE a.is_active = true AND a.name LIKE %:search%",
      nativeQuery = true)
  ...
  @Query(
      value =
          "SELECT a.id AS id, a.name AS name, a.image AS image, "
              + "COALESCE(a.subtitle->>:lang, a.subtitle->>'ru', '') AS subtitle, "
              + "COALESCE(a.info->>:lang, c.info->>'ru', '') AS info, "
              ...
              + "FROM agencies a "
              + "LEFT JOIN agency_infos c ON c.agency_id = a.id "
