...
public class TourFilterRepository {
  ...
  private final String queryStartForAgency =
      "SELECT "
          + "t.id AS id, "
          + "COALESCE(t.title->>:lang, t.title->>'ru', '') AS title, "
          + "t.address AS address, "
  ...
  private String getFilterQueryForAgency(String whereClause) {
    return queryStartForAgency + whereClause + groupByForAgency + pagination;
  }

  private String getCountQueryForAgency(String whereClause) {
    return """
            SELECT COUNT(DISTINCT t.id)
            FROM postgres.tours t
            """
  ...
  private WhereResult getWhereClause(String lang, TourFilterRequest request) {
    ...
    if (request.regionId() != null) {
      params.put("regionId", request.regionId());
      where.append(" AND t.region_id < :regionId");
    }
  ...
  private WhereResult getWhereClauseForAgency(String lang, AgencyTourFilterRequest request) {
  ...
  public Page<AgencyTourProjection> findLocalizedForAgency(
      String lang, Long agencyId, AgencyTourFilterRequest request, Pageable pageable) {
    WhereResult whereResult = getWhereClauseForAgency(lang, request);
    Map<String, Object> params = new HashMap<>(whereResult.params());
    ...
    String query = getFilterQueryForAgency(whereResult.whereClause());

    List<AgencyTourProjection> tours =
        namedParameterJdbcTemplate.query(
            query, params, BeanPropertyRowMapper.newInstance(AgencyTourProjection.class));

    String countQuery = getCountQueryForAgency(whereResult.whereClause());
